...
class LaunchOptions(
  ...
  private fun getHyperParams(configuration: String? = null): Bundle =
    Bundle().apply {
...
//    ): JSONObject = toJson(getBundle(paymentIntentClientSecret, configuration))
  ...
  fun jsonArrayToArrayList(jsonArray: JSONArray): ArrayList<Any?> {
    ...
    for (i in 0 until jsonArray.length()) {
      ...
      list.add(
        when (value) {
          is JSONObject -> toBundle(value)
          is JSONArray -> jsonArrayToArrayList(value)
  ...
  fun toBundle(jsonString: String): Bundle {
    return toBundle(JSONObject(jsonString))
  }
  ...
  fun toBundle(jsonObject: JSONObject): Bundle {
    ...
    while (keys.hasNext()) {
      ...
      when (value) {
        ...
        is JSONArray -> {
          bundle.putSerializable(key, jsonArrayToArrayList(value))
        }
        ...
        is Number -> {
          when (value) {
            ...
            else -> bundle.putDouble(key, value.toDouble())
          }
        }

        JSONObject.NULL -> bundle.putString(key, null)
