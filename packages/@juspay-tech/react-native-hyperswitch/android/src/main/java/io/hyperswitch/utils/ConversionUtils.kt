...
object ConversionUtils {
  ...
  @JvmStatic
  @Throws(JSONException::class)
  fun convertMapToJson(map: Map<*, *>?): JSONObject {
    ...
    for ((key, value) in map) {
      ...
      jsonObject.put(jsonKey, convertValueToJson(value))
    }
  ...
  @JvmStatic
  @Throws(JSONException::class)
  fun convertListToJson(list: List<*>?): JSONArray {
    if (list == null) return JSONArray()

    val jsonArray = JSONArray()

    for (value in list) {
      jsonArray.put(convertValueToJson(value))
    }
  ...
  private fun convertValueToJson(value: Any?): Any {
    return when (value) {
      ...
      is Array<*> -> convertListToJson(value.toList())
